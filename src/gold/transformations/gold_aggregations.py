...
def calculate_gold_metrics(imu_df, encoder_df, gps_df):
    ...
    master_df = (
        imu_df
        .join(encoder_df, on="timestamp", how="left")
        .join(gps_df, on="timestamp", how="left")
    )
    ...
    return (
        master_df
        .withColumn("velocity_mps", col("total_ticks_per_sec") * lit(TICKS_TO_METERS))
        .withColumn("prev_velocity", lag("velocity_mps", 1).over(window_spec))
        .withColumn("prev_timestamp", lag("timestamp", 1).over(window_spec))
        .withColumn(
            "delta_t_sec", 
            coalesce(col("timestamp").cast("double") - col("prev_timestamp").cast("double"), lit(0.0))
        )
        .withColumn(
            "acceleration_mps2",
            when(col("delta_t_sec") > 0, (col("velocity_mps") - col("prev_velocity")) / col("delta_t_sec"))
            .otherwise(lit(0.0))
        )
...
def gold_kinetic_master():
    ...
    imu = spark.read.table("iot_data.silver.silver_imu_history").filter("__END_AT IS NULL").drop("__START_AT", "__END_AT")
    gps = spark.read.table("iot_data.silver.silver_gps_history").filter("__END_AT IS NULL").drop("__START_AT", "__END_AT")
    encoder = spark.read.table("iot_data.silver.silver_encoder_history").filter("__END_AT IS NULL").drop("__START_AT", "__END_AT")
