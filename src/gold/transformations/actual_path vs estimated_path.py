...
def generate_spatial_features(gold_df):
    ...
    session_marked_df = gold_df.withColumn("prev_ts", lag("timestamp", 1).over(time_window)).withColumn(
        "is_new_session",
    ...
    session_id_df = session_marked_df.withColumn("session_id", F.sum("is_new_session").over(time_window)).drop(
        "is_new_session"
    )
    ...
    # -------------------------------------------------------------------------
    unbounded_window = (
        Window.partitionBy("session_id").orderBy("timestamp").rowsBetween(Window.unboundedPreceding, Window.currentRow)
    )
    ...
    time_df = session_id_df.withColumn(
        "dt_sec", coalesce(col("timestamp").cast("double") - col("prev_ts").cast("double"), lit(0.0))
    )
    ...
    return (
        ...
        .withColumn(
            ...
            (col("latitude") - F.first("latitude").over(unbounded_window)) * lit(METERS_PER_DEGREE),
        )
        .withColumn(
            "feature_actual_path_y",
            (col("longitude") - F.first("longitude").over(unbounded_window)) * lit(METERS_PER_DEGREE * 0.7),
        )
